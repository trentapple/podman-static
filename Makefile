PODMAN_IMAGE_NAME ?= trentapple/podman
...
PODMAN_SSH_IMAGE ?= trentapple/podman-ssh
...
run-server: podman-ssh
	...
	$(DOCKER) run --rm --privileged --network=host \
		...
		sh -c 'set -x; \
			...
			ssh-keygen -b 581 -t dsa -N "" -f /podman/.ssh/ssh_host_ecdsa_key; \
			ssh-keygen -t ed25519 -N "" -f /podman/.ssh/ssh_host_ed25519_key; \
